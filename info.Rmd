...
# setting
# set location to to 'uppsala','uppsala-slu' or 'umea'
loc <- "uppsala-slu"
```
...
if(loc=="uppsala-slu") {
  dfr <- data.frame(venue_lat=59.815055,venue_lon=17.663513,venue_address="Biblioteket, Almas Allé 12, Campus Ultuna, SLU, 75007 Uppsala,Sweden",description="Use the [UL website](www.ul.se) or the [UL app](https://www.ul.se/en/tickets/how-to-buy-a-ticket/the-UL-app/) for bus and train services around Uppsala. The bus stop is called *Uppsala Campus Ultuna* and bus options are 4, 12, 100, 101 and 188. Bus tickets can be purchased in the app or directly from the driver using a credit card.",stringsAsFactors=F)
}
