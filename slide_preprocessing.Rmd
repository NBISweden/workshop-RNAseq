...
#tpm
g <- read.delim("./data/mouse_genes.txt",header=T,stringsAsFactors=F)
...
ggplot(pcamat2,aes(PC1,PC2,colour=Group))+
  geom_point()+
  theme_bw()+
  theme(panel.border=element_blank(),
        axis.ticks=element_blank())
...
# plotly pca
library(plotly)
pcamat2$tt <- paste0("<b>ID: </b>",pcamat2$SampleID,"\n",
                   "<b>Name: </b>",pcamat2$SampleName)
p1 <- plot_ly(pcamat2,x=~PC1,y=~PC2,z=~PC3,text=~tt,hoverinfo="text")
add_markers(p1,color=~Group)
```
