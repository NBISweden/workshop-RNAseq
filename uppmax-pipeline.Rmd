...
<div class="instruction">
**<i class="fas fa-exclamation-circle"></i> Run this only if you canceled your job before lunch**
</div>
...
```
$ salloc -A g2019011 -t 04:00:00 -p core -n 1 --no-shell --reservation=g2019011_1 &
```
...
$ cp -r /sw/share/compstore/courses/ngsintro/uppmax_pipeline_exercise/data /proj/g2019011/nobackup/<username>/uppmax_pipeline_exercise
```

Have a look in `/proj/g2019011/nobackup/<username>/uppmax_pipeline_exercise`:

```
$ cd /proj/g2019011/nobackup/<username>/uppmax_pipeline_exercise
$ ll
```
...
First, we'll do an analysis manually without scripts, just to get the hang of it. Then we'll start writing scripts for it!
...
<i class="fas fa-exclamation-circle"></i> The export command affects only the terminal you type it in. If you have 2 terminals open, only the terminal you typed it in will have a modified path. If you close that terminal and open a new one, it will not have the modified path.
...
```
$ cd /proj/g2019011/nobackup/<username>/uppmax_pipeline_exercise/exomeSeq
```
...
To find SNPs we have to have a reference to compare our data with. The same reference genome as you aligned to is the one to use.
...
SLURM is also a bit strict when it comes formalities. It requires that you specify which program should be used to run the submitted script file. The standard program for this is bash, but we have to specify it on the first line of the script non the less.
...
The next step is to do a complete RNAseq analysis. The steps involved start off just like the exome analysis, but has a few extra steps.
The goal of this part is to successfully run the pipeline using the queue system. To do this, you must construct the commands for each step, combine them in a script, include the SLURM options, and submit it. Much like what we did in the previous step, but with some complications.
...
<i class="fas fa-lightbulb"></i> The samples are filtered and aligned individually.

For the really fast ones: Continue with this tutorial on [Advanced Linux](advanced-linux.html) if you still have time left on the lab.
