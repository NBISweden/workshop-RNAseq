...
data <- read.csv("data/counts_filtered.csv",header=TRUE,stringsAsFactors=FALSE,row.names=1)
...
gene_names <- as.character ( annot[match(rownames(data),annot[,"ensembl_gene_id"]),"hgnc_symbol"] )
...
logcpm <- log2( cpm + 1)
if(!file.exists("data/cpm.csv")) write.csv(cpm ,"data/cpm.csv",row.names=T)
```
