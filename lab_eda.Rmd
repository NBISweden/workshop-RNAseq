...
```{r,results='hide',block.title=TRUE,fig.height=6,fig.width=10}
Znorm <- t(apply(logcpm,1,function(x) scale(x,center=T,scale=T)))
colnames(Znorm) <- colnames(logcpm)

{
  mypar(1,2,mar=c(5,3,2,1))
  boxplot(t(logcpm[1:30,]),ylim=c(0,12),las=2,col="grey",main="logCPMraw_data",cex=.2)
...
pheatmap(logcpm[top_var, ],scale="row",
         color=colorRampPalette(c("navy","white","firebrick"))(90),
