...
cpm <- edgeR::cpm(data)
logcpm <- log2( cpm + 1)
write.csv(cpm ,"data/cpm.csv",row.names=T)
```
...
barplot(leading_PC1[15:1], las=2,horiz = T,cex.names = .8,cex.axis = .8,yaxs="i")
abline(v=0,lwd=2)
barplot(leading_PC2[15:1], las=2,horiz = T,cex.names = .8,cex.axis = .8,yaxs="i")
abline(v=0,lwd=2)
```
...
pvc <- pvclust( data = logcpm[top_var[1:100],] , method.dist = "correlation", method.hclust = "complete")
mypar()
