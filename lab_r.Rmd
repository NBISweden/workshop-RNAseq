...
```{r,include=FALSE}
weblink <- "https://raw.githubusercontent.com/NBISweden/course_rnaseq_slu/master/"
```
...
R is a programming language for statistical computing, and data wrangling. It is open-source, widely used in data science, has a wide range of functions and algorithms for graphing and data analyses. Here is a quick introduction to working with R. You can follow this tutorial by copy-pasting/typing commands from below. Alternatively, you can download this RMarkdown file ``r paste0(weblink,"lab_r.Rmd")``, open it in R/RStudio and execute the commands. 
...
Text files are the most commonly used input. Text files can be read in using the function `read.table`. You can read in your own tabular file if you have. Else we have a sample file to use: **iris.txt** at this link ``r paste0(weblink,"data/iris.txt")`` that you can download and save to a directory `data` under your current working directory..

```{r,eval=FALSE}
dfr <- read.table("./data/iris.txt",header=TRUE,stringsAsFactors=F)
```
