...
Feel free to contact us if you encounter any issue. Once you have joined the course in Canvas, you can do so via the Discussions section entitled "installation and pre-course related issues". For any issue related to creating your account on Canvas, please email us at edu.rnaseq [at] nbis.se.
...
Make sure that you have the latest [**Zoom**](https://zoom.us/download) installed because we will use it a lot during the workshop for the lecture and groups discussions.

If you already have Zoom installed, you can also update it by clicking in `Check for Updates ...` button in the main top menu.
...
cd ~/Desktop/course
wget https://raw.githubusercontent.com/NBISweden/workshop-RNAseq/master/env.yml
...
To download the `env.yml` file using the command on Terminal:
...
# MacOSX
curl -o env.yml https://raw.githubusercontent.com/NBISweden/workshop-RNAseq/master/envMacOSX.yml
```

After this, you should have a file named `env.yml` in your directory (it does not matter where). Next, type:
...
Go to your home folder and caunch the course environment:
...
```{sh,eval=FALSE,chunk.title=TRUE}
wget -r -nH --cut-dirs=2 --no-parent --reject="index.html*" https://export.uppmax.uu.se/uppstore2017171/workshops/RNAseq
```
...
You now have 3 main folders there with data.
...
```{sh,eval=FALSE,chunk.title=TRUE}
mkdir ~/RNAseq   #Run this line if you have not downloaded the data for the course yet
cd ~/RNAseq

for i in lab_annotation.Rmd lab_dge.Rmd lab_download.Rmd lab_eda.Rmd lab_functional.Rmd lab_preprocessing.Rmd lab_geoquery.Rmd
do
